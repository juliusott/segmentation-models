...
# GLOBALS
NUM_EPOCHS = 10
BATCH_SIZE = 2  # large Batch size leads to a memory issue

# choose model
models = [DeepLab(), SegResNet(), SegNet(), Autoencoder()]
model = models[1]  # 0=DeepLab, 1=SegResNet, 2= SegNet, 3=Autoencoder
...
train(num_epochs=NUM_EPOCHS, train_dataloader=train_dataloader, model=model, input_path="./trained_models/segresnet_nn.pt")
validation(val_dataset, val_dataloader, model, BATCH_SIZE, path_to_model="./trained_models/segresnet_nn.pt")
